...
You will use the `data_download.R` file to download your data from the Google Sheet that you have established for your capstone project. Use the `googlesheets4` package to access the data. Do not clean the data, but save it in the `data/raw` folder using the `write_csv()` function. Give the the a meaningful name that describes your project.
